...
declare -A SUB
SUB[AT]='s,http://127.0.0.1:8000,http://trust-anchor.org:8000,g'
SUB[RP]='s,http://127.0.0.1:8001,http://relying-party.org:8001,g'
SUB[OP]='s,http://127.0.0.1:8002,http://cie-provider.org:8002,g'
SUB[WTA]='s,http://127.0.0.1:8000,http://wallet.trust-anchor.org:8005,g'
...
for svc in federation_authority relying_party provider; do
  # JSON dump
  sed -e "${SUB[AT]}" -e "${SUB[RP]}" -e "${SUB[OP]}" \
